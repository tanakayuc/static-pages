...
const contentChecks = [
  ...
  ["roadmap.html", "第2章の対象箇所"],
  ["roadmap.html", "第2章 コンセプト設計"],
  ["roadmap.html", "3. オファー構築"],
  ["roadmap.html", "本命商品オファー"],
  ["roadmap.html", "第3章の対象箇所"],
  ["roadmap.html", "オファー構築"],
  ...
  ["tasks-bonus.html", "課題/特典案内文"],
  ["tasks-bonus.html", "課題・特典の全体整理"],
  ["tasks-bonus.html", "Day別提出課題"],
  ["tasks-bonus.html", "コンプリート特典"],
  ["tasks-bonus.html", "1つ上に戻る"],
...
const generatedDetailPages = fs.existsSync(root)
  ? fs.readdirSync(root).filter((name) => /^(optin-after-mail|traffic-mail|line-fixed|line-normal|line-official|sales-mail|sales-oc|sales-line)-\d+\.html$/.test(name)).sort()
  : [];
...
const activeContentChecks = [
  ...
  ["assets.html", "制作物一覧"],
  ["lp.html", "集客で作る素材"],
  ["optin-lp-copy.html", "キャッチコピー"],
  ["optin-lp-copy.html", "本編動画（オプト前VSL）"],
  ["optin-lp-copy.html", "ヘッドバナー"],
  ["optin-lp-copy.html", "5日間ライブトレーニングプログラム"],
  ["head.html", "LPヘッドデザイン指示書"],
  ["script-opening.html", "オプト前VSL台本"],
  ["optin-after-mails.html", "オプトイン後メルマガ一覧"],
  ["thank-you-copy.html", "サンキューページ原稿"],
  ["referral-copy.html", "紹介用文章"],
  ...
  ["value.html", "価値提供で作る素材"],
  ["line.html", "固定投稿"],
  ["live-scripts.html", "ライブ台本"],
  ["tasks-bonus.html", "課題/特典案内文"],
  ["sales-page.html", "販売で作る素材"],
  ["sales-letter.html", "セールスレター原稿"],
  ["purchase-complete.html", "購入完了ページ原稿"],
];

for (const [file, snippet] of activeContentChecks) {
  const ok = read(file).includes(snippet);
  if (!ok) fail(`${file} missing content: ${snippet}`);
}

const valueHtml = read("value.html");
if (/href="live-scripts\.html"[^>]*>[^<]*(?:<[^>]+>[^<]*){0,4}課題\/特典案内文/.test(valueHtml)) {
  fail("value.html should not send 課題/特典案内文 to live-scripts.html");
}
if (/href="live-scripts\.html"[^>]*>[^<]*(?:<[^>]+>[^<]*){0,4}課題と特典/.test(valueHtml)) {
  fail("value.html should not send 課題と特典 to live-scripts.html");
}
const roadmapHtml = read("roadmap.html");
for (const label of ["チャレンジ課題＆特典設計", "特典の作成（実体）", "チャレンジ課題のアウトプットフォーム"]) {
  const pattern = new RegExp(`${label}[\\s\\S]{0,700}live-scripts\\.html`);
  if (pattern.test(roadmapHtml)) fail(`roadmap.html should not send ${label} to live-scripts.html`);
}
...
for (const snippet of [
  ...
  if (read("tasks-bonus.html").includes(snippet)) {
    fail(`tasks-bonus.html should not include source/internal snippet: ${snippet}`);
  }
}
...
for (const snippet of [
  ...
  if (read("optin-lp-copy.html").includes(snippet)) fail(`optin-lp-copy.html should not include source/meta snippet: ${snippet}`);
}

for (const file of ["target.html", "concept.html", "config.html", "research.html", "offer.html"]) {
...
for (const file of generatedOutputPages) {
  const html = read(file);
  if (/https?:\/\//.test(html)) fail(`${file} should not expose completed URLs in generated materials`);
  if (/20\d{2}-[0-9]{1,2}-[0-9]{1,2}|20\d{2}年[0-9０-９]{1,2}月[0-9０-９]{1,2}日|[0-9０-９]{1,2}月[0-9０-９]{1,2}日/.test(html)) {
    fail(`${file} should not expose actual delivery dates in generated materials`);
  }
  for (const snippet of ["該当URL", "該当LP", "完成URL", "取得元", "relation_elements", "実ページスクリーンショット", "Produced by", "Generated by", "Licensed to", "Security Trace"]) {
    if (html.includes(snippet)) fail(`${file} should not expose source/meta snippet: ${snippet}`);
  }
}
