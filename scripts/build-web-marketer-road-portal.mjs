...
<section class="panel"><h2>ファネル全体</h2><div class="funnel">${funnelSteps.map(([label]) => `<div class="funnel-step">${esc(label)}</div>`).join("")}</div><p class="quote">LPで動画を見せてオープンチャットへ参加してもらい、Day5で公式LINEへ移動した方にだけ期間限定セールスレターを公開する構成です。</p></section>
`}));
