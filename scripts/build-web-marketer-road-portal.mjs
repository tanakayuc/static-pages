...
const vslPlacementRows = [
  ...
  ["オプト後VSL", "サンキューページ（登録直後）", "登録後の軽い案内ではなく、OC参加、Day1着席、チャレンジ参加期待値を高める本気のVSL。5分前後、長くても5〜10分を目安にする。", "サンキューページ原稿、5〜10分VSL台本、OC参加CTA"],
...
const roadmapPhases = [
  {
    ...
    items: [
      ...
      {
        name: "VSL配置を選択する",
        make: "チャレンジローンチの入口を、LP上のオプトインVSL、登録後サンキュー上のオプト後VSL、または併用のどれで組むか決める。オプト後VSLは5分前後、長くても5〜10分を目安にする。",
...
<section class="panel"><h2>VSL配置の選択</h2><p class="note">チャレンジローンチでは、VSLをオプトイン前に置くのか、オプトイン後に置くのか、併用するのかを全体設計で先に決めます。オプト後VSLは登録後の軽い案内ではなく、5分前後、長くても5〜10分でOC参加、Day1着席、完走率を押し上げるためのビデオセールスレターとして扱います。</p>${vslPlacementTable()}</section>
...
<section class="panel"><h2>VSL配置の選択肢</h2><p class="note">一気通貫モードでは、最初の全体設計でオプトインVSL、オプト後VSL、併用のどれで組むかを確認します。オプト後VSLは5分前後、長くても5〜10分を目安にし、サンキューページで正式参加とDay1着席への熱量を作ります。</p>${vslPlacementTable()}</section>
