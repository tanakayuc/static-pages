...
const detailedAssetRows = [
  ...
  ["VSL台本", "オプトインVSLまたはオプト後VSLとして、次CTAを担う5〜10分の説得動画。", "script-opening.html"],
...
const vslPlacementRows = [
  ["オプト前VSLパターン", "オプトインLP上部（登録前）", "コールド広告 × 無料チャレンジ × OC/ライブ必須。登録率だけでなく後工程の質を守りたいとき。", "LP原稿、VSL台本、視聴連動の遅延CTA指示"],
  ["オプト後VSLパターン", "サンキューページ（登録直後）", "登録後の軽い案内ではなく、OC参加、Day1着席、チャレンジ参加期待値を高める本気のVSL。5分前後、長くても5〜10分を目安にする。", "サンキューページ原稿、5〜10分VSL台本、OC参加CTA"],
...
const activeFunnelFormat = {
  ...
  nextLiveDay: "Day2",
  salesPattern: "セールスレター（販売ページ）による直接販売パターン",
};
...
const acquisitionPatternRows = [
  {
    ...
    chooseWhen: "コールド広告、重いチャレンジ、LINEオープンチャット参加が必須のとき。登録率だけでなく後工程の質も守りたい。",
  ...
  },
  {
    ...
    chooseWhen: "LPは短く登録を優先し、登録直後に正式参加、OC参加、Day1着席への期待値を高めたいとき。",
...
const challengePatternRows = [
  ["2チャレ", "Day1〜Day2", "Day2", "短期で参加ハードルを下げる。2本のライブ台本、課題、配信を作る。"],
  ["3チャレ", "Day1〜Day3", "Day2 / Day3", "教育量と完走率のバランスを取る。3本のライブ台本、課題、配信を作る。"],
  ["4チャレ", "Day1〜Day4", "Day2 / Day3 / Day4", "販売前に教育量を増やす。4本のライブ台本、課題、配信を作る。"],
  ["5チャレ", "Day1〜Day5", "Day2 / Day3 / Day4 / Day5", "今回の標準形。5日間のライブ、課題、特典、販売接続を作る。"],
];
...
const individualProductionRows = [
  ["LP/ページ", "オプトインLP、サンキュー、セールスページ、購入完了ページ", "lp.html", "工程5 / 工程8"],
  ["ステップメール", "オプトイン自動返信、販売期メルマガ、件名と配信タイミング", "stepmail.html", "工程6 / 工程8"],
  ["LINE配信", "全体ポータル、固定投稿、通常配信、公式LINE販売期配信", "line.html", "工程6 / 工程8"],
  ["ライブ台本", "Day1〜Day5台本、課題、特典、アーカイブ導線", "live-scripts.html", "工程4 / 工程7"],
  ["設計シート", "コンセプト、プロフィール、コンフィグ、リサーチ、オファー", "sheets.html", "工程2 / 工程3"],
...
const roadmapPhases = [
  {
    ...
    items: [
      ...
      {
        name: "VSL配置を選択する",
        make: "チャレンジローンチの入口を、オプト前VSLパターン、オプト後VSLパターン、または併用のどれで組むか決める。オプト後VSLは5分前後、長くても5〜10分を目安にする。",
      ...
      {
        name: "何チャレかを決める",
        make: "2チャレ〜5チャレのどれで価値提供を行うか、次のライブがDay2〜Day5のどこなのかを決める。",
      ...
      {
        name: "販売形態を選択する",
        make: "個別説明会、セミナーから個別説明会、セミナー直接販売、セールスレター直接販売の4パターンから選ぶ。",
  ...
  },
  {
    ...
    items: [
      {
        name: "オプトインLP原稿",
        make: "誰に、何を約束し、なぜ今参加するのかをLP本文として整理する。オプトインVSLを採用する場合は動画前提の本文にする。",
...
function normalizeOutputTerms(text = "") {
  return String(text)
    ...
    .replaceAll("サンキュー動画", "オプト後VSL")
    .replaceAll("ティーアップ動画", "オプト後VSL")
    .replaceAll("オプトインVSL", "オプト前VSL")
...
<div class="format-summary">
  <span>現在のサンプル設定</span>
  <strong>${esc(activeFunnelFormat.acquisitionPattern)} / ${esc(activeFunnelFormat.challengePattern)} / 次ライブ ${esc(activeFunnelFormat.nextLiveDay)} / ${esc(activeFunnelFormat.salesPattern)}</strong>
</div>
...
function acquisitionPatternTable() {
  return `<table class="asset-table"><thead><tr><th>集客パターン</th><th>配置</th><th>選ぶ条件</th><th>作る素材</th></tr></thead><tbody>${acquisitionPatternRows.map((row) => `<tr class="${row.label === activeFunnelFormat.acquisitionPattern ? "active-row" : ""}"><td><strong>${esc(row.label)}</strong></td><td>${esc(row.placement)}</td><td>${esc(row.chooseWhen)}</td><td>${pills(row.assets)}</td></tr>`).join("")}</tbody></table>`;
}

function challengePatternTable() {
  return `<table class="asset-table"><thead><tr><th>何チャレ</th><th>ライブ範囲</th><th>次ライブ候補</th><th>作る素材の考え方</th></tr></thead><tbody>${challengePatternRows.map(([label, range, next, detail]) => `<tr class="${label === activeFunnelFormat.challengePattern ? "active-row" : ""}"><td><strong>${esc(label)}</strong></td><td>${esc(range)}</td><td>${esc(next)}</td><td>${esc(detail)}</td></tr>`).join("")}</tbody></table>`;
}

function salesPatternTable() {
  return `<table class="asset-table"><thead><tr><th>販売形態</th><th>選ぶ条件</th><th>作る素材</th><th>導線</th></tr></thead><tbody>${salesPatternRows.map((row) => `<tr class="${row.label === activeFunnelFormat.salesPattern ? "active-row" : ""}"><td><strong>${esc(row.label)}</strong></td><td>${esc(row.chooseWhen)}</td><td>${esc(row.output)}</td><td>${esc(row.nodes.join(" → "))}</td></tr>`).join("")}</tbody></table>`;
}
...
function individualProductionTable() {
...
<tr><th>販売分岐</th><td>Day5後に、販売ページ、個別説明会、セミナー導線のいずれかへ分岐する。</td></tr>
</tbody></table>
</section>
...
<section class="panel"><h2>ファネルフォーマット選択</h2><p class="note">制作を始める前に、集客のVSL配置、何チャレか、次ライブの位置、販売形態を決めます。ここで選んだ型に合わせて、工程表の中で作る素材が変わります。</p>${funnelFormatBoard()}</section>
...
pages.set("visual-report.html", page({
  ...
  body: `
<section class="panel"><h2>今回のファネルフォーマット</h2><p class="note">この図は、集客・価値提供・販売の3フェーズで、今どの素材を作っているのかを示すためのベースです。今回のサンプルはオプト前VSLパターン、5チャレ、次ライブ Day2、セールスレター直接販売として表示しています。</p>${funnelFormatBoard()}</section>
<section class="panel"><h2>集客パターンの出し分け</h2><p class="note">集客ページでVSLを先に見せるのか、登録後のサンキューページでオプト後VSLを見せるのかで、LP、サンキュー、CTA、台本の作り方が変わります。オプト後VSLは、登録後の軽い案内ではなくビデオセールスレターとして扱います。</p>${acquisitionPatternTable()}</section>
<section class="panel"><h2>何チャレと次ライブ</h2><p class="note">2チャレ〜5チャレのどれを選ぶか、次のライブがDay2〜Day5のどこかによって、価値提供フェーズの素材数と表示が変わります。</p>${challengePatternTable()}</section>
<section class="panel"><h2>販売形態4パターン</h2><p class="note">販売工程は、商品単価や説明量に合わせて変わります。今回のサンプルはセールスレター（販売ページ）による直接販売パターンです。</p>${salesPatternTable()}</section>
...
<section class="panel"><h2>ファネルフォーマット確認</h2><p class="note">第1章の全体設計で、VSL配置、何チャレか、次ライブの位置、販売形態を決めます。ここで選んだ内容に合わせて、第5章のLP/サンキュー/VSL、第7章のライブ台本、第8章の販売素材が変わります。</p>${funnelFormatBoard()}</section>
...
pages.set("assets.html", page({
  ...
  title: "制作物一覧",
  eyebrow: "制作物",
  lead: "制作物は、工程順とは別に、集客フェーズの素材、価値提供フェーズの素材、販売ページの素材で確認します。",
  body: `<section class="panel"><h2>制作物管理の考え方</h2><p class="note">工程表は「作る順番」を決める本体です。一方で、制作物一覧は「どの役割の素材か」を見る場所にします。LP、VSL、ヘッド指示、自動返信のように工程がまたがる素材は、無理に2層で並べず、3階層で管理します。</p><div class="checklist">
<div class="checkitem"><strong>集客フェーズの素材</strong><p>SNS、広告、オプトインLP、VSL、サンキュー、自動返信など、登録と正式参加までの素材。</p></div>
<div class="checkitem"><strong>価値提供フェーズの素材</strong><p>オープンチャット、ステップメール、LINE、Day1〜Day5ライブ、課題、特典など、参加維持と納得感を作る素材。</p></div>
<div class="checkitem"><strong>販売ページの素材</strong><p>販売前メッセージ、セールスレター、販売期配信、個別説明会ページ、購入完了ページなど、購入判断と申込後を支える素材。</p></div>
...
<section class="panel"><h2>工程表との関係</h2><p class="note">たとえばLP周辺だけでも、オプトインLP、サンキューページ、VSL台本、ヘッド指示、自動返信メールに分かれます。工程表では作る順番を追い、制作物一覧ではそれらを集客フェーズの素材としてまとめ、各詳細ページへ紐づけます。</p></section>`}));

pages.set("lp.html", page({
  file: "lp.html",
  title: "集客素材",
  eyebrow: "制作物",
  lead: "集客フェーズの素材に必要なオプトインLP、VSL、ヘッド指示、サンキュー、自動返信メールを確認します。",
  body: `<section class="panel"><h2>このページで扱う範囲</h2><p class="note">ここでは、集客フェーズの素材として田中祐一AIが作るものを確認します。オプトインLP原稿、LPヘッド指示書、オプト前VSL/オプト後VSL台本、サンキューページ原稿、自動返信メール原稿をまとめます。セールスレターや購入完了ページは販売ページの素材として別ページで扱います。</p></section>
<section class="panel"><h2>集客パターンの配置</h2><p class="note">このサンプルは、LP上に動画を置くオプト前VSLパターンです。全体設計でオプト後VSLパターンに切り替える場合は、サンキューページ原稿とOC参加CTAを中心に作ります。</p>${acquisitionPatternTable()}</section>
<section class="panel"><h2>集客フェーズの素材</h2><p class="note">次の素材を作ってから、実装担当やUTAGE設定へ渡します。実装先は後工程で設定し、ここでは原稿・台本・指示書を成果物として扱います。</p>${productionAssetTable(lpAssetRows)}</section>
<section class="panel"><h2>集客フェーズの制作カード</h2><p class="note">自動返信まで完成すると、選択したVSL配置に合わせてオプトインLPを実装し、LINEオープンチャットへ参加させる最低限の導線が動かせます。</p><div class="grid-3">
...
pages.set("sales-page.html", page({
  ...
  title: "セールスページ原稿",
  eyebrow: "制作物",
  lead: "販売ページの素材として必要なセールスレター原稿、ヘッド指示書、販売期配信、個別説明会ページ、購入完了ページ原稿を確認します。",
  body: `<section class="panel"><h2>販売形態4パターン</h2><p class="note">販売形態によって必要素材が変わります。今回のサンプルはセールスレター（販売ページ）による直接販売パターンですが、商品や相談の必要性に応じて個別説明会型、セミナー型へ切り替えます。</p>${salesPatternTable()}</section>
<section class="panel"><h2>販売ページの素材</h2><p class="note">ここでは、田中祐一AIが販売工程で作る素材を管理します。セールスレター原稿、デザイン指示書、販売期メール/LINE、個別説明会ページ原稿、購入完了ページ原稿を分けて作ります。</p>${productionAssetTable(salesAssetRows)}</section>
